...
namespace TFSEventsProcessor.Providers
{
    ...
    public class TfsFieldLookupProvider : IFieldLookupProvider
    {
        ...
        /// </summary>
        private bool showMissingFieldNames;
        ...
        public TfsFieldLookupProvider(WorkItem workItem, List<WorkItemChangedAlertDetails> alertItems, string changedBy, bool showMissingFieldNames)
        {
        ...
        public virtual string LookupWorkItemFieldValue(string fieldName)
        {
            ...
            try
            {
                ...
                value = this.workItem.Fields[fieldName].Value.ToString();
            }
        ...
        public string GetAllWorkItemFields(string title)
        {
            ...
            foreach (Field field in this.workItem.Fields)
            {
                logger.Debug(string.Format("GetAllWorkItemFields  for {0}", field.ReferenceName));
                sb.AppendFormat("<tr><td>{0}</td><td>{1}</td></tr>", field.ReferenceName, field.Value);
            }
            sb.Append("</table>");
            return sb.ToString();
        }
        ...
        private string BuildEmailAddress(string name, string domain)
        {
            // on tfs 2012 we get the displayname as opposed to uid, so we to check
            var tmpName = this.GetUserIdFromDisplayName(name);
            if (string.IsNullOrEmpty(tmpName))
            {
                // could not do a lookup so use the original
                tmpName = name;
            }
            return string.Format("{0}@{1}", tmpName, domain);
        }
